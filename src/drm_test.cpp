...
int flip(int argc, char **argv)
{
	 ...
	 * 后面 drmModeSetCrtc 设置modes的模式要和这里一样否则会报-28 错误
	 * 该列表可以使用modetest -c 查看
	 * 见https://doc.embedfire.com/linux/rk356x/driver/zh/latest/linux_driver/framework_drm.html#id4
	 */ 
	create_fb(fd,connector->modes[12].hdisplay,connector->modes[12].vdisplay, 0xCDB38B, &buf[0]);	//创建显存和上色
	printf(" display size %d %d buf 1 size %d \n",connector->modes[12].hdisplay,connector->modes[12].vdisplay,buf[0].size);
	create_fb(fd,connector->modes[12].hdisplay,connector->modes[12].vdisplay, 0xBBFFFF, &buf[1]);	
	// create_fb(fd,1280,720, 0xCDB38B, &buf[0]);	//创建显存和上色
	// create_fb(fd,1280,720, 0xBBFFFF, &buf[1]);	
	// printf(" display size %d %d buf 1 size %d \n",connector->modes[0].hdisplay,connector->modes[0].vdisplay,buf[0].size);
	...
	int res = 0;
	res = drmModeSetCrtc(fd, crtc_id, buf[0].fb_id,	0, 0, &conn_id, 1, &connector->modes[12]);	//初始化和设置crtc，对应显存立即刷新
	printf("%d ,%s \n",res,strerror(res));
	res = drmModePageFlip(fd, crtc_id, buf[0].fb_id,DRM_MODE_PAGE_FLIP_EVENT, &crtc_id);
	printf("%d ,%s \n",res,strerror(res));
	// while (!terminate) {
